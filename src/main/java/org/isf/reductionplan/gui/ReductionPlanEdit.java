...
public class ReductionPlanEdit extends ModalJFrame  {
	...
	private JPanel buttonPanel;
		...
		private void addExamReduction() throws OHServiceException {
			...
			List<Exam> examArray = examManager.getExams();
		...
		private void loadDataFromObject() throws OHServiceException {
			...
			try {
				medicalReductionList = manager.getMedicalsForReductionPlan(this.reductionPlan.getId());
				examReductionList = manager.getExamsForReductionPlan(this.reductionPlan.getId());
				opeReductionList = manager.getOperationsForReductionPlan(this.reductionPlan.getId());
				othReductionList = manager.getOthersForReductionPlan(this.reductionPlan.getId());
		...
		private class ExamReductionModel extends DefaultTableModel {
			...
			public ExamReductionModel() throws OHException, OHServiceException {
				...
				manager = new ReductionplanBrowserManager();
				try {
		...
		private class OperationReductionModel extends DefaultTableModel {
			...
			public OperationReductionModel() throws OHException, OHServiceException {
				ReductionplanBrowserManager  manager = new ReductionplanBrowserManager();
				try {
		...
		private class MedicalReductionModel extends DefaultTableModel {
			...
			public Object getValueAt(int r, int c) {
				if (c == 0) {
					return medManager.getMedical(medicalReductionList.get(r).getId()).getDescription();
				} else if (c == -1) {
